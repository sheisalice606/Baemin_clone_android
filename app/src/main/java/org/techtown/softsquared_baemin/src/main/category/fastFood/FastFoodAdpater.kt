...
class FastFoodAdpater(val context: Context, private val items : ArrayList<FastFoodData>) :
        RecyclerView.Adapter<FastFoodAdpater.Holder>() {
    ...
    inner class Holder(itemView : View) : RecyclerView.ViewHolder(itemView) {
        ...
        val restaurantIcon = itemView.findViewById<ImageView>(R.id.fragment_fast_food_restaurant_icon)

        // 가게 아이콘
        val restaurant = itemView.findViewById<TextView>(R.id.fragment_fast_food_restaurant)

        // 가게 이름
        val newIcon = itemView.findViewById<ImageView>(R.id.fragment_fast_food_new)

        // 신규 아이콘
        val takeOut = itemView.findViewById<ImageView>(R.id.fragment_fast_food_take_out)

        // 포장 아이콘
        val score = itemView.findViewById<TextView>(R.id.fragment_fast_food_score)

        val lowCost = itemView.findViewById<TextView>(R.id.fragment_fast_food_low_cost)
        // 최소 주문 비용
        val deliveryTime = itemView.findViewById<TextView>(R.id.fragment_fast_food_delivery_time)
        // 배달 시간

        // 배달 시간 + 최소주문비용
        val deliveryTip = itemView.findViewById<TextView>(R.id.fragment_fast_food_tip)
        // 배달팁


        @SuppressLint("SetTextI18n")
        fun bind(item: FastFoodData, context: Context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        val view = LayoutInflater.from(context).inflate(R.layout.fragment_fastfood_recycler_item, parent, false)
        return Holder(view)
    }
