...
class PorkAdater(val context: Context, private val items : ArrayList<PorkData>) :
        RecyclerView.Adapter<PorkAdater.Holder>() {
    ...
    inner class Holder(itemView : View) : RecyclerView.ViewHolder(itemView) {
        ...
        val restaurantIcon = itemView.findViewById<ImageView>(R.id.fragment_pork_restaurant_icon)
        // 가게 아이콘
        val restaurant = itemView.findViewById<TextView>(R.id.fragment_pork_restaurant)
        // 가게 이름
        val newIcon = itemView.findViewById<ImageView>(R.id.fragment_pork_new)
        // 신규 아이콘
        val takeOut = itemView.findViewById<ImageView>(R.id.fragment_pork_take_out)
        // 포장 아이콘
        val score = itemView.findViewById<TextView>(R.id.fragment_pork_score)
        // 평점
        val lowCost = this.itemView.findViewById<TextView>(R.id.fragment_pork_low_cost)
        // 최소 주문 비용
        val deliveryTime = this.itemView.findViewById<TextView>(R.id.fragment_pork_delivery_time)
        // 배달 시간
        val deliveryTip = itemView.findViewById<TextView>(R.id.fragment_pork_tip)
        // 배달팁


        @SuppressLint("SetTextI18n")
        fun bind(item : PorkData, context: Context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        val view = LayoutInflater.from(context).inflate(R.layout.fragment_pork_recycler_item, parent, false)
        return Holder(view)
    }
