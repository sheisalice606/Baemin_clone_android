...
class MyPageAdapterBottom(val context : Context, private val items : ArrayList<MyPageDataBottom>) :
        RecyclerView.Adapter<MyPageAdapterBottom.Holder>(){

    inner class Holder(itemView : View) : RecyclerView.ViewHolder(itemView) {
        val text_main = itemView.findViewById<TextView>(R.id.before_login_my_page_list_2_text1)

        fun bind(item : MyPageDataBottom, context: Context) {
