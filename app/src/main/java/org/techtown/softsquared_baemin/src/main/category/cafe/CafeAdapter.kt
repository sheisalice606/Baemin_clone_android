...
class CafeAdapter(val context: Context, private val items : ArrayList<CafeData>) :
    ...
    inner class Holder(itemView : View) : RecyclerView.ViewHolder(itemView) {
        ...
        val restaurantIcon = itemView.findViewById<ImageView>(R.id.fragment_cafe_restaurant_icon)

        // 가게 아이콘
        val restaurant = itemView.findViewById<TextView>(R.id.fragment_cafe_restaurant)

        // 가게 이름
        val newIcon = itemView.findViewById<ImageView>(R.id.fragment_cafe_new)

        // 신규 아이콘
        val takeOut = itemView.findViewById<ImageView>(R.id.fragment_cafe_take_out)

        // 포장 아이콘
        val score = itemView.findViewById<TextView>(R.id.fragment_cafe_score)

        val lowCost = itemView.findViewById<TextView>(R.id.fragment_cafe_low_cost)
        // 최소 주문 비용
        val deliveryTime = itemView.findViewById<TextView>(R.id.fragment_cafe_delivery_time)
        // 배달 시간

        // 배달 시간 + 최소주문비용
        val deliveryTip = itemView.findViewById<TextView>(R.id.fragment_cafe_tip)
        // 배달팁


        @SuppressLint("SetTextI18n")
        fun bind(item: CafeData, context: Context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CafeAdapter.Holder {
        val view = LayoutInflater.from(context).inflate(R.layout.fragment_cafe_recycler_item, parent, false)
        return Holder(view)
    }
