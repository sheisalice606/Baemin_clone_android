...
class SteamedAdapter(val context: Context, private val items : ArrayList<SteamedData>) :
        RecyclerView.Adapter<SteamedAdapter.Holder>() {
    ...
    inner class Holder(itemView : View) : RecyclerView.ViewHolder(itemView) {
        ...
        val restaurantIcon = itemView.findViewById<ImageView>(R.id.fragment_steamed_restaurant_icon)
        // 가게 아이콘
        val restaurant = itemView.findViewById<TextView>(R.id.fragment_steamed_restaurant)
        // 가게 이름
        val newIcon = itemView.findViewById<ImageView>(R.id.fragment_steamed_new)
        // 신규 아이콘
        val takeOut = itemView.findViewById<ImageView>(R.id.fragment_steamed_take_out)
        // 포장 아이콘
        val score = itemView.findViewById<TextView>(R.id.fragment_steamed_score)
        // 평점
        val lowCost = this.itemView.findViewById<TextView>(R.id.fragment_steamed_low_cost)
        // 최소 주문 비용
        val deliveryTime = this.itemView.findViewById<TextView>(R.id.fragment_steamed_delivery_time)
        // 배달 시간
        val deliveryTip = itemView.findViewById<TextView>(R.id.fragment_steamed_tip)
        // 배달팁


        @SuppressLint("SetTextI18n")
        fun bind(item : SteamedData, context: Context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        val view = LayoutInflater.from(context).inflate(R.layout.fragment_steamed_recycler_item, parent, false)
        return Holder(view)
    }
